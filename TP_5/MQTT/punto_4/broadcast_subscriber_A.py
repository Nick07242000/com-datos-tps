...
def on_connect(client, userdata, flags, rc):
    if rc == 0:
        print("Cliente A conectado. Esperando Broadcasts.")
        client.subscribe(TOPIC_SUBSCRIBE, qos=1) 

def on_message(client, userdata, msg):
    payload = msg.payload.decode()
    print("\n" + "=" * 40)
    print(f"[{time.strftime('%H:%M:%S')}] 📥 CLIENTE A: ¡BROADCAST RECIBIDO!")
    print(f"Tópico: {msg.topic}")
    print(f"Payload: {payload}")
    print("=" * 40)

client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION1, "Broadcast_Subscriber_A")

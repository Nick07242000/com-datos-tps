...
def connect_mqtt():
    client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION1, "humidity_sensor")
...
def publish(client, room, humidity):
    result = client.publish(f"lan/sala{room}/sensor/hum", humidity, qos=1) # QoS 1 garantiza la llegada al menos una vez
    # Verifica si el mensaje fue enviado con éxito
    if result[0] == 0:
        print(f"[{time.strftime('%H:%M:%S')}] ✅ Humedad enviada: '{humidity}'")

...
def on_connect(client, userdata, flags, rc):
    if rc == 0:
        print("Client 2 conectado. Esperando Broadcasts.")
        client.subscribe(TOPIC_SUBSCRIBE, qos=1) 

def on_message(client, userdata, msg):
    payload = msg.payload.decode()
    print("\n" + "*" * 40)
    print(f"[{time.strftime('%H:%M:%S')}] 📥 CLIENTE 2: ¡BROADCAST RECIBIDO!")
    print(f"Tópico: {msg.topic}")
    print(f"Mensaje: {payload}")
    print("*" * 40)

# ÚNICA DIFERENCIA CLAVE: ID distinto
client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION1, "Client2_Broadcast_Sub") 
